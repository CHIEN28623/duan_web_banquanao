    ...
    <div class="container mx-auto w-[500px] py-10">
        ...
        <?php foreach ($users as $item) {
            ...
            <input type="text" name="phone_number" placeholder="Số điện thoại (bắt buộc)" class="block w-full border 
                border-gray-400 rounded px-4 py-2 mb-4" required value="<?= $item['phone_number'] ?>"/>
            <input type="text" name="address" placeholder="Địa điểm nhận hàng (bắt buộc)" class="block w-full border 
                border-gray-400 rounded px-4 py-2 mb-4" required value="<?= $item['address'] ?>"/>
            <div>
            <?php } ?>
            <input class="font-bold bg-blue-500 hover:bg-blue-600 text-white rounded px-4 py-2 w-full" name="thanhtoan" 
            type="submit" value="Thanh toán">
            </div>
        </form>    
    </div>
</div>