  ...
  <div class="flex flex-col shadow-md my-10 lg:flex-row">
    <div class="lg:w-3/4 bg-white px-10 py-10">
      <div class="flex justify-between border-b pb-8">
        <h1 class="font-semibold text-2xl">Chi tiết mã đơn <?= $order_id ?></h1>
      ...
      <?php foreach ($order_items as $item) { ?>
        <div class="flex items-center hover:bg-gray-100 -mx-8 px-6 py-5">
          ...
          <span class="text-center w-1/5 font-semibold text-sm ">
            Size <?= substr($item['size'], 5) ?>
          </span>
        </div>
      <?php } ?>
      <!-- product -->
      <a href="/duan1/site/order?list" class="flex font-semibold text-indigo-600 text-sm mt-10">
